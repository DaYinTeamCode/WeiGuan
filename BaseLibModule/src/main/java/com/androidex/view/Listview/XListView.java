...
import com.androidex.view.Listview.XListViewFooter.RetryLoadClickListener;
...
public class XListView extends ListView implements OnScrollListener {
    ...
    private boolean mIsPullReleaseScrollingTouchEnable = true;
    private int mLoadMorePosOffset = 0;

    /**
     * 自动加载
     */
    private boolean autoLoadMore = true;
    private String loadMoreMsg;

    public String getLoadMoreMsg() {
        return loadMoreMsg;
    }

    public void setLoadMoreMsg(String loadMoreMsg) {
        this.loadMoreMsg = loadMoreMsg;
        if(mFooterView != null){
            mFooterView.setRemindLoad(loadMoreMsg);
        }
    }
    ...
    private final static int SCROLLBACK_NONE = -1;
    private final static int SCROLLBACK_HEADER = 0;
    private final static int SCROLLBACK_HEADER_FORECE = 1;
    private final static int SCROLLBACK_FOOTER = 2;
    ...
    private boolean mForcePullRefreshStoped = false;
    ...
    private void initWithContext(Context context) {
        ...
        mFooterView.setRetryLoadClickListener(new RetryLoadClickListener() {
            @Override
    ...
    public TextView getHeaderPullTextView(){
        ...
        return mHeaderView.getTipTextView();
    }

    public TextView getFooterFailedTextView(){

        return mFooterView.getTipFailedTextView();
    }

    public TextView getFooterLoadingTextView(){

        return mFooterView.getTipLoadingTextView();
    }

    public ProgressWheel getFooterLoadingView(){

        return mFooterView.getTipLoadingView();
    }

    public void setProgressColor(int color){

        mHeaderView.getTipLoadingView().setBarColor(color);
        mFooterView.getTipLoadingView().setBarColor(color);
    }
    ...
    public void setRemindLoad(String text) {

        mFooterView.setRemindLoad(loadMoreMsg);
    }

    public void addHeaderViewLast(View v){
    ...
    public void addFooterViewLast(View v) {

        if (v != null)
            mFooterView.addView(v, mFooterView.getChildCount());
    }

    public void addFooterViewIndex(View v,int index) {

        if (v != null)
            mFooterView.addView(v, index);
    }
    ...
    public void stopRefresh() {
        ...
        if (mScrollBack == SCROLLBACK_HEADER_FORECE) {

            mForcePullRefreshStoped = true;
        } else {
    ...
    public void setLoadMorePosOffset(int offset){

        if(offset < 0)
            offset = 0;

        mLoadMorePosOffset = offset;
    }
...
//		ToastUtil.showToast("delta : " + delta  +" visibale height : "+ mHeaderView.getVisiableHeight());
        ...
        if (mEnablePullRefresh && !mPullRefreshing) {
            ...
            if (mListViewListener != null && delta > 0) {

                if(delta > 30){

                    mListViewListener.overScrollTop();
                }
            }
        }
    ...
    public void setBottomMargin(int marginBottom) {
        if (mFooterView != null) {
            mFooterView.setBottomMargin(marginBottom);
        }
    }
    ...
    public void ForceRefresh(boolean scrollAble) {

        startRefresh(true);
         if(scrollAble){

             mScrollBack = SCROLLBACK_HEADER_FORECE;
...
//		}
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (!isPullRefreshEnable() || (!mIsPullReleaseScrollingTouchEnable && !mScroller.isFinished()) || mScrollBack == SCROLLBACK_HEADER_FORECE)
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (mScrollBack == SCROLLBACK_HEADER_FORECE && mForcePullRefreshStoped) {
    ...
    public void onScroll(AbsListView view, int firstVisibleItem,
        ...
        if (mListViewListener != null && !mPullLoading /*&& !mPullRefreshing*/
            ...
            //LogMgr.d("xlv","hotRec load more ,cur scroll pos="+(visibleItemCount + firstVisibleItem)+", totalItemcount="+totalItemCount);
            int realOffset = totalItemCount;
            if(totalItemCount > mLoadMorePosOffset)
                realOffset = totalItemCount - mLoadMorePosOffset;

            if (visibleItemCount + firstVisibleItem >= realOffset) {
                ...
                if(isAutoLoadMore()){
                    ...
                    mFooterView.setState(mFooterView.STATE_REMIND);
                }
            }
        }
