 ...
 * 		如果destoryItem正常完成，此时Fragment的生命周期是:onStop() -> onDestoryView() ->最后onDestory()，并销毁Fragment对象；
 * 		如果destoryItem未正常完成，则什么都不做;
 *
 * 当需要再次使用的时候，通过instantiateItem()，获取Fragment：
 * 		如果之前正常destoryItem，创建Fragment对象，调用Fragment对象的onCreateView -> onActivityCreate() -> onStart
 ...
 * 		如果destoryItem正常完成，此时Fragment的生命周期是:onStop() -> onDestoryView() -> 最后onDestory()，
...
public abstract class ExFragmentPagerStateAdapter<T> extends FragmentStatePagerAdapter{
    ...
    public ExFragmentPagerStateAdapter(Context context, FragmentManager fmtMgr) {
